...
object SupervisorStrategy extends SupervisorStrategyLowPriorityImplicits {
   ...
   * INTERNAL API
   */
  private[actors] def sort(in: Iterable[CauseDirective]): immutable.Seq[CauseDirective] =
  ...
  private[actors] def withinTimeRangeOption(withinTimeRange: Duration): Option[Duration] =
    if (withinTimeRange.isFinite && withinTimeRange >= Duration.Zero) Some(withinTimeRange)
    else None

  private[actors] def maxNrOfRetriesOption(maxNrOfRetries: Int): Option[Int] =
    if (maxNrOfRetries < 0) None else Some(maxNrOfRetries)

  private[actors] val escalateDefault = (_: Any) => Escalate
}
...
abstract class SupervisorStrategy {
  ...
  /**
   * This method is called to act on the failure of a child: restart if the
