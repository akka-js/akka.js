...
class MessageDispatcher(
   ...
   */
  protected[actors] def createMailbox(actor: ActorCell): Mailbox =
   ...
   * INTERNAL API
   */
  protected[actors] def register(actor: ActorCell) {
    //addInhabitants(+1)
  }
   ...
   * INTERNAL API
   */
  protected[actors] def unregister(actor: ActorCell) {
    //addInhabitants(-1)
   ...
   */
  protected[actors] def suspend(actor: ActorCell): Unit = {
   ...
   */
  protected[actors] def resume(actor: ActorCell): Unit = {
    val mbox = actor.mailbox
   ...
   * INTERNAL API
   */
  protected[actors] def systemDispatch(receiver: ActorCell, invocation: SystemMessage) = {
   ...
   * INTERNAL API
   */
  protected[actors] def dispatch(receiver: ActorCell, invocation: Envelope) = {
    val mbox = receiver.mailbox
    val s = receiver.self
   ...
   * INTERNAL API
   */
  protected[actors] def registerForExecution(mbox: Mailbox,
   ...
   * INTERNAL API
   */
  protected[actors] def throughput: Int = 10

  /**
   * INTERNAL API
   */
  protected[actors] def throughputDeadlineTime: Duration =
    Duration.fromNanos(1000)

  /**
   * INTERNAL API
   */
  @inline protected[actors] final val isThroughputDeadlineTimeDefined =
