  ...
  "An Actor" must {
    ...
    "invoke preRestart, preStart, postRestart when using OneForOneStrategy" in {
      filterException[ActorKilledException] {
        ...
        val restarter = Await.result((supervisor ? restarterProps)/*.mapTo[ActorRef]*/, timeout.duration).asInstanceOf[ActorRef]
    ...
    "not invoke preRestart and postRestart when never restarted using OneForOneStrategy" in {
      ...
      val a = Await.result((supervisor ? props)/*.mapTo[ActorRef]*/, timeout.duration).asInstanceOf[ActorRef]
