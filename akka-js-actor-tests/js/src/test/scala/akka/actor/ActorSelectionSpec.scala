...
class ActorSelectionSpec extends AkkaSpec/*("akka.loglevel=DEBUG")*/ with DefaultTimeout {
  ...
  "An ActorSystem" must {
    ...
    "select actors by their path" in {
      ...
      // @note IMPLEMENT IN SCALA.JS identify(system child "c2" child "c21") should be(Some(c21)) // test Java API
      identify(system / Seq("c2", "c21")) should be(Some(c21))

      // import scala.collection.JavaConverters._
      // @note IMPLEMENT IN SCALA.JS identify(system descendant Seq("c2", "c21").asJava) // test Java API
    }
  ...
  "An ActorSelection" must {
    ...
    /* @note IMPLEMENT IN SCALA.JS (ActorSystem name doesn't reflect ActorSelectionSpec) */
    //  "print nicely" in {
