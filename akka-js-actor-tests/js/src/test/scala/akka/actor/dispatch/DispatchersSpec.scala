...
object DispatchersSpec {
  ...
  //@Todo ConcurrentLinkedQueue cant find replacemeent that is consistent
  class DoublingMailbox(owner: Option[ActorRef]) extends UnboundedQueueBasedMessageQueue {
    final val queue = new akka.dispatch.AbstractNodeQueue[Envelope]()
...
class DispatchersSpec extends AkkaSpec(DispatchersSpec.config) with ImplicitSender {
  ...
  "Dispatchers" must {
    ...
    // @Todo Dspatchers cannot be changed easily
    /*
