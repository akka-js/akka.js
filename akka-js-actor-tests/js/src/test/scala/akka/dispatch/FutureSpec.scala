...
trait FutureMatchers {
  class CompleteMatcher extends BeMatcher[Future[_]] {
    def apply(f: Future[_]) =
      MatchResult(
        f.isCompleted,
        "future was complete",
        "future was not complete"
      )
  }
  val completed = new CompleteMatcher
}
...
object FutureSpec {
  ...
  def ready[T](f:Future[T], atMost: Duration): f.type  =
    try Await.ready(f, atMost) catch {
      case t: TimeoutException ⇒ throw t
      case e if NonFatal(e)    ⇒ f //swallow
    }
...
class FutureSpec extends AkkaSpec with Checkers with FutureMatchers with BeforeAndAfterAll with DefaultTimeout {
  import FutureSpec._
  ...
  "A Future" when {
    ...
    "being tested" must {
      ...
      "fold" in {
        Await.result(Future.fold((1 to 10).toList map { i ⇒ Future(i) })(0)(_ + _), timeout.duration) should ===(55)
      }
      ...
      "fold with an exception" in {
        filterException[IllegalArgumentException] {
          ...
          intercept[Throwable] { Await.result(Future.fold(futures)(0)(_ + _), timeout.duration) }.getMessage should ===("shouldFoldResultsWithException: expected")
        }
      }
/*
      ...
      "reduce results" in {
        val futures = (1 to 10).toList map { i ⇒ Future(i) }
        assert(Await.result(Future.reduce(futures)(_ + _), timeout.duration) === 55)
      }
      ...
      "reduce results with Exception" in {
        filterException[IllegalArgumentException] {
          ...
          intercept[Throwable] { Await.result(Future.reduce(futures)(_ + _), timeout.duration) }.getMessage should ===("shouldReduceResultsWithException: expected")
        }
      }
     ...
     /* "not deadlock with nested await (ticket 1313)" in {
      ...
      "re-use the same thread for nested futures with batching ExecutionContext" in {
        ...
        FutureSpec.ready(f, timeout.duration)
        // TODO re-enable once we're using the batching dispatcher
        // failCount.get should ===(0)
      }


      "dup re-use the same thread for nested futures with batching ExecutionContext" in {
        val failCount = new java.util.concurrent.atomic.AtomicInteger
        val f = Future(()) flatMap { _ ⇒
          val originalThread = Thread.currentThread
          // run some nested futures
