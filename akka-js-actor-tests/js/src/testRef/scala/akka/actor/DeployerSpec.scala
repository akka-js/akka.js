...
object DeployerSpec {
  val deployerConfString = """
      ...
      }//@Todo ConfigFactory parse
      """
  val deployerConf = ConfigFactory.parseString(deployerConfString/*, ConfigParseOptions.defaults*/)
...
class DeployerSpec extends AkkaSpec(DeployerSpec.deployerConf) {
  "A Deployer" must {
   ...
   //@Todo ConfigException.WrongType
    ...
    "detect invalid deployment path" in {
      val e = intercept[InvalidActorNameException] {
        val invalidDeployerConf = ConfigFactory.parseString(s"""
          ${DeployerSpec.deployerConfString}
            akka.actor.deployment {
              /gul/ubåt {
                router = round-robin-pool
                nr-of-instances = 2
