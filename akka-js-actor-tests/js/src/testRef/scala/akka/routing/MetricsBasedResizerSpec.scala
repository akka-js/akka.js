...
class MetricsBasedResizerSpec extends AkkaSpec(ResizerSpec.config) with DefaultTimeout with ImplicitSender {
  ...
  "MetricsBasedResizer reportMessageCount" must {
    ...
    "start an underutilizationStreak when not fully utilized" in {
      ...
      //resizer.record.underutilizationStreak.get.start.isBefore(LocalDateTime.now(Clock.system(ZoneId.systemDefault))) shouldBe true
      //resizer.record.underutilizationStreak.get.start.isAfter(LocalDateTime.now(Clock.system(ZoneId.systemDefault))) shouldBe true
    }

    "stop an underutilizationStreak when fully utilized" in {
      val resizer = DefaultOptimalSizeExploringResizer()
      /*resizer.record = ResizeRecord(
        underutilizationStreak = Some(UnderUtilizationStreak(start = LocalDateTime.now(Clock.system(ZoneId.systemDefault)), highestUtilization = 1)))
      */
      val router = TestRouter(routees(2))
      router.sendToAll(await = true)
    ...
    /*"leave the underutilizationStreak start date unchanged when not fully utilized" in {
     // val start: LocalDateTime = LocalDateTime.now(Clock.system(ZoneId.systemDefault))
      val resizer = DefaultOptimalSizeExploringResizer()
     resizer.record = ResizeRecord(
    ...
    "leave the underutilizationStreak highestUtilization unchanged if current utilization is lower" in {
      val resizer = DefaultOptimalSizeExploringResizer()
      /*resizer.record = ResizeRecord(
        underutilizationStreak = Some(UnderUtilizationStreak(start = LocalDateTime.now(Clock.system(ZoneId.systemDefault)), highestUtilization = 2)))
      */
      val router = TestRouter(routees(2))
...
*/
  ...
  "MetricsBasedResizer resize" must {
    "downsize to close to the highest retention when a streak of underutilization started downsizeAfterUnderutilizedFor" in {
      ...
      /*esizer.record = ResizeRecord(underutilizationStreak = None
        Some(UnderUtilizationStreak(start = LocalDateTime.now(Clock.system(ZoneId.systemDefault)), highestUtilization = 8)))*/
