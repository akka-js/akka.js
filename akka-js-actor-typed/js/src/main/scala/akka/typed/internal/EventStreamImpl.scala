...
private[typed] class EventStreamImpl(private val debug: Boolean)(implicit private val timeout: Timeout) extends EventStream {
  ...
  def startDefaultLoggers(system: ActorSystemImpl[Nothing]) {
    ...
    try {
      val defaultLoggers = system.settings.Loggers match {
        case Nil     ⇒ classOf[akka.event.JSDefaultLogger].getName :: Nil
  ...
  private def addLogger(
    ...
    // val logger = akka.util.Reflect.instantiate(clazz) // clazz.newInstance()
    ...
    // TODO Scala.Js this should become again configurable in some way...
    val actor = system.systemActorOf(akka.typed.JSDefaultLogger.initialBehavior, name, DispatcherFromConfig(system.settings.untyped.LoggersDispatcher))
    import AskPattern._
    ...
    // (actor, logChannel)
    // Scala.Js => async implementation
    implicit val ec = system.executionContext
    val logChannel = try actor.flatMap(_ ? (Logger.Initialize(this, _: ActorRef[ActorRef[LogEvent]]))) catch {
      case ex: TimeoutException ⇒
        publish(Warning(logName, this.getClass, "Logger " + name + " did not respond within " + timeout + " to InitializeLogger(bus)"))
        throw ex
    }
    logChannel.map{ lc =>
      AllLogLevels filter (level >= _) foreach (l ⇒ subscribe(lc, classFor(l)))
      publish(Debug(logName, this.getClass, "logger " + name + " started"))
      (actor, lc)
    }
  }
