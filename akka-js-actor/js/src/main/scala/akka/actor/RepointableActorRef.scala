...
private[akka] class RepointableActorRef(
  ...
  */
  ...
  final def swapCell(next: Cell): Cell = {
    val old = _cellDoNotCallMeDirectly
    _cellDoNotCallMeDirectly = next
    old
  }
  
  final def swapLookup(next: Cell): Cell = {
    val old = _lookupDoNotCallMeDirectly
    _lookupDoNotCallMeDirectly = next
    old
  }
  ...
  */
  ...
  def sendMessage(msg: Envelope): Unit = {
    ...
    if (lock.tryLock(timeout.length, timeout.unit)) {
      ...
      /** @note IMPLEMENT IN SCALA.JS } finally lock.unlock()
    } else {
