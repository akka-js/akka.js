 ...
 */
 trait TActor extends Actor {
   def receive = new Receive {
     ...
     def apply(o: Any) {
       if (Thread.currentThread ne TestActorRefSpec.thread)
         TestActorRefSpec.otherthread = Thread.currentThread
       recv(o)
     }
   }
 ...
 class ReplyActor extends TActor {
   ...
   def receiveT = {
     case "complexRequest" ⇒ {
       replyTo = sender()
       val worker = TestActorRef(Props(new WorkerActor()))
       worker ! "work"
     }
...
class TestActorRefSpec extends AkkaSpec("disp1.type=Dispatcher") with BeforeAndAfterEach with DefaultTimeout {
  ...
  "A TestActorRef should be an ActorRef, hence it" must {
    ...
    "support reply via sender()" in {
      val serverRef = TestActorRef(Props(new ReplyActor()))
...
*/
    ...
    // TODO -> check and eventually re-enable these tests
