...
@InternalApi private[akka] class StubbedActorContext[T](val path: ActorPath, currentBehaviorProvider: () => Behavior[T])
  ...
  private val substituteLoggerFactory = org.slf4j.LoggerFactory
  ...
  def logEntries: List[CapturedLogEvent] = {
    ...
    //     CapturedLogEvent(
    //       level = evt.getLevel,
    //       message = evt.getMessage, //MessageFormatter.arrayFormat(evt.getMessage, evt.getArgumentArray).getMessage,
    //       cause = Option(evt.getThrowable),
    //       marker = Option(evt.getMarker))
    //   }
    //   .toList
    List[CapturedLogEvent]()
  }
