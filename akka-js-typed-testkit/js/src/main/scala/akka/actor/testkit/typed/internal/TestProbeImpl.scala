...
private[akka] object TestProbeImpl {
  ...
  private def testActor[M](queue: java.util.ArrayDeque[M], terminations: java.util.ArrayDeque[Terminated]): Behavior[M] =
    Behaviors
...
private[akka] final class TestProbeImpl[M](name: String, system: ActorSystem[_])
  ...
  private val queue = new java.util.ArrayDeque[M]
  // new LinkedBlockingDeque[M]
  private val terminations =  new java.util.ArrayDeque[Terminated]
  // new LinkedBlockingDeque[Terminated]
  ...
  private def receiveOne_internal(max: FiniteDuration): Option[M] = {
    val message = Option(internal_pollFirst(max))
 ...
 private def internal_pollFirst(max: FiniteDuration): M = {
  ...
  private def expectTerminated_internal[U](actorRef: ActorRef[U], max: FiniteDuration): Unit = {
    ...
    val message = terminations.pollFirst
    // val message =
