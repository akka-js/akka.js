...
trait TestKitBase {
   ...
   * guardian
   */
  // Verify why we need to explicit the type Akka.JS
  def spawn[T](behavior: Behavior[T]): ActorRef[T] =
    Await.result(system ? (SpawnActorAnonymous(behavior, (_ : ActorRef[ActorRef[T]]))), timeoutDuration)
  ...
  def spawn[T](behavior: Behavior[T], name: String): ActorRef[T] =
    Await.result(system ? (SpawnActor(name, behavior, (_ : ActorRef[ActorRef[T]]))), timeoutDuration)
