...
private[akka] final class TestProbeImpl[M](name: String, system: ActorSystem[_]) extends JavaTestProbe[M] with ScalaTestProbe[M] {
  ...
  // I have to change the interface of TestProbe to async
  override def expectTerminated[U](actorRef: ActorRef[U], max: FiniteDuration): Future[Unit] = {
    import scala.concurrent._

    have to rewrite this in unblocking style
