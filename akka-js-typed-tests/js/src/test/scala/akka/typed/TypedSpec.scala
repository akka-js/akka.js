...
class TypedSpecSetup extends org.scalatest.FunSuite /*RefSpec*/ with Matchers with BeforeAndAfterAll with ScalaFutures with TypeCheckedTripleEquals {
...
abstract class TypedSpec(val config: Config) extends TypedSpecSetup {
  import TypedSpec._
  import AskPattern._
  // for some reason it does not look properly working on JUnit ...
  ...
  trait StartSupport {
    ...
    def start[T](behv: Behavior[T]): ActorRef[T] = {
      ...
      // not sure why it is needed in scala.js and not in scala
  ...
  def await[T](f: Future[T]): T = // Await.result(f, timeout.duration * 1.1)
    akka.testkit.Await.result(f, timeout.duration * 1.1)
