...
trait ActorLogging {
  ...
  object log {
    def log(msg: String): Unit = js.Dynamic.global.console.log(msg)
    def info(msg: String): Unit = js.Dynamic.global.console.info(msg)
    def error(msg: String): Unit = js.Dynamic.global.console.error(msg)
  }
...
class Listener extends Actor with ActorLogging {
  ...
  def receive = {
    case Progress(percent) =>
      log.info(s"Current progress: $percent %")
      if (percent >= 100.0) {
...
class Worker extends Actor with ActorLogging {
  ...
  val counterService = context.actorOf(Props(new CounterService), name = "counter")
  val totalCount = 51
...
class CounterService extends Actor {
   ...
   */
  def initStorage() {
    storage = Some(context.watch(context.actorOf(Props(new Storage), name = "storage")))
